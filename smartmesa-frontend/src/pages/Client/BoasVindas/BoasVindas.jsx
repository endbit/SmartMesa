...
export default function BoasVindas() {
    ...
    // 🔍 valida sessão via QR
    useEffect(() => {
        async function loadSession() {
            try {
                setLoading(true);

                const res = await api.get(`/sessions/table/${sessionToken}`);
                ...
                console.error(err);
                setError("Mesa inválida ou sessão não encontrada.");
            } finally {
                setLoading(false);
            }
        }
    ...
    const handleEnter = () => {
        ...
        navigate(`/menu/${session.sessionToken}/cardapio`);
    };
        ...
        <div className="min-h-screen relative overflow-hidden bg-linear-to-br from-zinc-950 via-stone-900 to-neutral-950 flex items-center justify-center px-5">
            ...
            <div className="w-full max-w-md relative z-10">
                ...
                <div className="bg-stone-900/70 backdrop-blur-2xl border border-stone-700/60 rounded-3xl p-6 shadow-2xl">
                    ...
                    <button
                        onClick={handleEnter}
                        className="w-full mt-6 h-14 rounded-2xl bg-linear-to-r from-amber-500 to-red-500 text-white font-bold"
                    >
                        Entrar no Cardápio
